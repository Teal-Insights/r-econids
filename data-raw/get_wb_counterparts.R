...
# Fetch countries and regions from World Bank World Development Indicators API ----
...
geographies_wdi <- geographies_raw[[2]] |> 
  ...
  distinct() |> 
  mutate(geography_type = if_else(geography_iso3 %in% aggregates, "Region", "Country"))

# Fetch counterparts from World Bank International Debt Statistics API  ----

url_ids <- "https://api.worldbank.org/v2/sources/6/counterpart-area?per_page=32500&format=json"
...
counterparts_ids_enriched <- counterparts_ids |> 
  left_join(bind_rows(geographies_wdi, geographies_manual), join_by(counterpart_name == geography_name)) 
...
counterparts_ids_cleaned <- counterparts_ids_enriched |> 
  ...
  mutate(counterpart_name = if_else(!is.na(geography_name), geography_name, counterpart_name)) |> 
